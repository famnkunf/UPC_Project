...
#         self.PIN = Button(10)
        self.PIN = board.D18
        self.sensor = adafruit_dht.DHT22(self.PIN, use_pulseio=True)
#         self.sensor = DHT(18, False)
#         self.humidity = None
    ...
    def get_value(self):
        while True:
            try:
                ...
                print(e)
        self.humidity, self.temperature = Adafruit_DHT.read_retry(sensor=self.sensor_type, pin=self.PIN)
#         MAX_TRY = 15
#         i = 0
#         while True:
...
class DHT:
    ...
    def read(self):
        ...
        # pull down to low
        self.__send_and_sleep(0, 0.018)
#         self.__send_and_sleep(1, 0.03)
        # change to input using pull up
        self.PIN.close()
        self.PIN = Button(self.__pin, pull_up=True, bounce_time=None)
...
#             self.PIN.wait_for_press()
            current = self.PIN.is_pressed
            data.append(current)
            if last != current:
                unchanged_count = 0
                last = current
